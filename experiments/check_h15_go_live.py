#!/usr/bin/env python3
"""Check v57/H15 go-live gates from a canonical_eval JSON artifact.

This script is deliberately stricter than the earlier fine-tuning gates because
it answers a production question: should a candidate replace v42 as the live
semantic interviewer?
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    results = []
    for name, (op, threshold) in H15_GATES.items():
    ...
    if args.json:
        ...
        print(f"H15 go-live check: {out['model_id']}  anchor={out['self_anchor']}")
        print(f"Verdict: {out['verdict']}")
        for r in results:
            status = "PASS" if r["passed"] else "FAIL"
            value = "missing" if r["value"] is None else f"{r['value']:.4f}"
            print(f"  {status:<4} {r['gate']:<42} {value} {r['op']} {r['threshold']}")
